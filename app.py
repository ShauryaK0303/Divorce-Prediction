...
def predict_divorce(*args):
    user_inputs = np.array([arg[0] for arg in args]).reshape(1, -1)
    prediction = model.predict(user_inputs)
    return "Likely Divorce" if prediction[0] == 1 else "Unlikely Divorce"

inputs = [gr.components.CheckboxGroup(options, label=question,type="index") for question in questions]
output = gr.components.Label()
interface = gr.Interface(fn=predict_divorce, inputs=inputs, outputs=output, title="Divorce Prediction Questionnaire")
